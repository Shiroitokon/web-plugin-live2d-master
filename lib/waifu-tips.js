...
(function(){
    ...
    $.each(result.click, function (index, tips){
        $(document).on("click", tips.selector, function (event){
            var text = tips.text;
            if(Array.isArray(tips.text)) text = tips.text[Math.floor(Math.random() * tips.text.length + 1)-1];
            text = text.render({text: $(this).text()});
            showMessage(text, 3000);
...
function isInArea(event, selector) {
  var e = document.querySelector(selector);
  var x=event.clientX; 
  var y=event.clientY; 
  var divx1 = e.offsetLeft; 
  var divy1 = e.offsetTop; 
  var divx2 = e.offsetLeft + e.offsetWidth; 
  var divy2 = e.offsetTop + e.offsetHeight; 
